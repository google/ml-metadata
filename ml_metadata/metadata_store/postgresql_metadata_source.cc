...
absl::Status PostgreSQLMetadataSource::ConvertResultToRecordSet(
  ...
  for (int i = 0; i < num_rows; i++) {
    ...
    for (int j = 0; j < num_cols; j++) {
      ...
      new_record.add_values(PQgetisnull(res, i, j) ? kMetadataSourceNull
                                                   : PQgetvalue(res, i, j));
    }
...
absl::Status PostgreSQLMetadataSource::BeginImpl() {
  return RunPostgresqlStatement(kBeginTransaction);
}

absl::Status PostgreSQLMetadataSource::CommitImpl() {
  return RunPostgresqlStatement(kCommitTransaction);
}

absl::Status PostgreSQLMetadataSource::RollbackImpl() {
  return RunPostgresqlStatement(kRollbackTransaction);
}
