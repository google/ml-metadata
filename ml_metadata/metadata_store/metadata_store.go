...
// PutExecution inserts or updates an Execution and its input and output artifacts and
...
//
// It returns an error if
// a) no artifact or execution exists with the given id,
// b) artifact or execution's type id or property type does not align with the stored ones.
// c) the event.type field is UNKNOWN.
func (store *Store) PutExecution(e *mdpb.Execution, aep []*ArtifactAndEvent) (ExecutionID, []ArtifactID, error) {
	...
	return ExecutionID(resp.GetExecutionId()), rst, err
}
