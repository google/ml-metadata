...
tensorflow::Status ThreadRunner::Run(Benchmark& benchmark) {
  int64 workloads_size = benchmark.workloads_size();
  for (int workload_index = 0; workload_index < workloads_size;
       ++workload_index) {
    WorkloadBase* workload = benchmark.workload(workload_index);
    ThreadStats thread_stats_list[num_threads_];
    int64 op_per_thread =
        benchmark.workload_num_operations(workload_index) / num_threads_;
    ...
    TF_RETURN_IF_ERROR(workload->SetUp(set_up_store.get()));
    {
      ...
      for (int64 i = 0; i < num_threads_; ++i) {
        pool.Schedule([op_per_thread, workload, mlmd_config = mlmd_config_, i,
                       &thread_stats_list, &total_done]() {
          // Each thread uses a different MLMD client instance to talk to
          // the same back-end.
          std::unique_ptr<MetadataStore> store;
          CreateMetadataStore(mlmd_config, &store);
          int64 start_index = op_per_thread * i;
          thread_stats_list[i].Start();
          ...
          while (work_items_index < start_index + op_per_thread) {
            ...
            thread_stats_list[i].Update(op_stats, total_done);
          }
          thread_stats_list[i].Stop();
        });
      }
    }
