...
class FillTypes : public Workload<FillTypeWorkItemType> {
 ...
 protected:
  ...
  // For updating, checks if the number of current type is less than the
  // update number of operations first. If that's true, makes up and inserts
  // some new types in the db for update purpose. Then, the list of work
  // items(FillTyesRequests with update config.) will be generated by adding new
  // fields to the existed types(may includes some new types inserted in the
  // make up step) inside db. The number of added new fields will be generated
  // w.r.t. the uniform distribution.
